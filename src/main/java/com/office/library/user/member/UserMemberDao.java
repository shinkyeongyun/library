...
public class UserMemberDao {
	...
	public UserMemberVo selectUser(UserMemberVo userMemberVo) {
		...
		try {
			userMemberVos = jdbcTemplate.query(sql, new RowMapper<UserMemberVo>() {
				...
				@Override
				public UserMemberVo mapRow(ResultSet rs, int rowNum) throws SQLException {// Resultset으로 값을 받고 rowNum만큼
																							// 반복
					UserMemberVo userMemberVo = new UserMemberVo();// 객체에 담아서(set) 반환
			...
			if (!passwordEncoder.matches(userMemberVo.getU_m_pw(), userMemberVos.get(0).getU_m_pw()))
				userMemberVos.clear(); // 아이디가 일치하는 회원을 조회한 후, 조회된 회원의 비밀번호와 사용자가 입력한 비밀번호와 일치하는지 비교해서 일치하지않으면 삭제한다.
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public int updateUserAccount(UserMemberVo userMemberVo) {
		...
		String sql = "UPDATE tbl_user_member SET u_m_name = ?, u_m_gender = ?, "
				+ "u_m_mail = ?, u_m_phone = ?, u_m_mod_date = NOW() " + "WHERE u_m_no = ?";
