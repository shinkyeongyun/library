...
public class BookDao {
	...
	public int insertBook(BookVo bookVo) {		//DB생성시 한글추가에 에러가 나온다. DEFAULT CHARSET=utf8;를 테이블생성시 넣어줄것
	...
	public int updateRentalBook(int rb_no) {
		...
		String sql = "UPDATE tbl_rental_book SET rb_end_date = NOW() WHERE rb_no = ?";	//대출번호(rb_no)와 일치하는 도서의 반납일을 업데이트합니다
	...
	public int updateBook(int b_no) {
		...
		String sql = "UPDATE tbl_book SET b_rantal_able = 1 WHERE b_no = ?";	//도서번호(b_no)가 일치하는 도서의 대출가능을 1로 변경합니다.
	...
	public void updateHopeBookResult(int hb_no) {
		...
		String sql = "UPDATE tbl_hope_book SET hb_result = 1, hb_result_last_date = NOW() "
				+ "WHERE hb_no = ?";		
